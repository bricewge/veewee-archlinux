...
#  :hostiocache => 'off',
  ...
  :boot_cmd_sequence => [
    ...
    "#{root_password}<Enter>",
    "#{root_password}<Enter>",
    "echo 'Server = http://archlinux.polymorf.fr/$repo/os/$arch' > /etc/pacman.d/mirrorlist<Enter>",
    'systemctl start sshd.service<Enter><Wait><Wait><Wait><Wait><Wait>',
  ],
...
#  :ssh_guest_port    => '22',
  ...
  :params => {
    #:PACMAN_REFLECTOR_ARGS => '--verbose -c "France" -n 5 --sort rate --save /etc/pacman.d/mirrorlist',
  }
})
